...
const names=JSON.parse(fs.readFileSync('design/redesign/assignments/core.json')).map(x=>x.name);
for(let batch=0;batch<6;batch++){
 ...
 await sharp(Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1190" height="1080"><rect width="1190" height="1080" fill="white"/>${cells}</svg>`)).png().toFile(`/tmp/core-${batch}.png`);
}
console.log('Rendered four 56px/26px contact sheets under /tmp/core-*.png');
