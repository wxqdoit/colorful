...
for(const entry of entries){if(!art.has(entry.name))throw Error('Missing '+entry.name);const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--project-art-detail, #aa8bcf)" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round">${art.get(entry.name)}</svg>`;fs.writeFileSync(`assets/regular/${entry.name}.svg`,roundSVG(svg).source)}
